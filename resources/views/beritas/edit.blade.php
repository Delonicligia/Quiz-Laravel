...
@section('content')
<div class="max-w-4xl mx-auto bg-white shadow-xl rounded-xl border border-gray-100">
    <!-- Header Section -->
    <div class="px-8 py-6 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-indigo-50">
        <div class="flex items-center space-x-3">
            ...
            <div>
                ...
                <p class="text-sm text-gray-600 mt-1">Perbarui informasi berita di bawah ini</p>
            </div>
        </div>
    </div>
    ...
    <!-- Form Content -->
    <div class="p-8">
        <form action="{{ route('beritas.update', $beritas->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="judul" id="judul" value="{{ old('judul', $beritas->judul) }}" required
            ...
            <div class="space-y-2">
                ...
                <textarea name="isi" id="isi" rows="6" required
                          ...
                          class="w-full px-4 py-3 text-gray-900 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all duration-200 hover:border-gray-400 resize-none">{{ old('isi', $beritas->isi) }}</textarea>
                @error('isi')
                    <div class="flex items-center mt-2 text-sm text-red-600">
                        <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path>
            ...
            <div class="space-y-2">
                ...
                @if($beritas->foto)
                    <div class="mb-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                        ...
                        <img src="{{ asset('storage/' . $beritas->foto) }}" alt="Foto berita" class="w-32 h-24 object-cover rounded-md border border-gray-300">
                        <p class="text-xs text-gray-500 mt-2">Upload foto baru untuk mengganti foto yang ada</p>
                    </div>
                @endif
                ...
                <div class="relative">
                    ...
                    <div class="flex items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-lg hover:border-indigo-400 hover:bg-indigo-50 transition-all duration-200">
                        <div class="text-center">
                            ...
                            <p class="text-sm text-gray-600 font-medium">{{ $beritas->foto ? 'Klik untuk mengganti foto' : 'Klik untuk upload foto' }}</p>
                    ...
                    <select name="user_id" id="user_id" required
                        ...
                        <option value="" class="text-gray-500">-- Pilih Penulis --</option>
                         @if(isset($users) && $users->count() > 0)
                            ...
                            @endforeach
                        @else
                            <option value="{{ $beritas->user_id }}" selected>
